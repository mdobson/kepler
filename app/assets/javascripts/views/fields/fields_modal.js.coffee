class Studycache.Views.FieldsModal extends Backbone.View
  ...
  saveData:->
    ...
    columns = input.find("#cols").val()
    rows = input.find("#rows").val()
    @model.model.model.set(canblank:canblank, question:question, helptext:helptext, datapoint:datapoint, defaults:defaults, video:video, numberscale:numberscale, columns:columns, rows:rows)
    input.find(".modal").addClass("hide")
    input.find("#backdrop").remove()
